...
/* ===== API ===== */
async function fetchJSON(url, opts){ const res=await fetch(url,opts); if(!res.ok) throw new Error(res.status+' '+res.statusText); return res.json(); }
...
(function dragAndResize(){
  ...
  function onDown(x,y){
    ...
    startLeft=rect.left; startTop=rect.top; startX=x; startY=y;
