...
console.log('VoxPro Manager script loaded');
...
// Load media from multiple possible sources
async function loadMedia() {
    try {
        ...
        // First try direct list-media endpoint
        try {
            const response = await fetch(API_CONFIG.LIST_MEDIA);
            const result = await response.json();
            
            log('List media response:', result);
            
            if (result && result.data && Array.isArray(result.data)) {
                mediaList = result.data;
                log(`Loaded ${mediaList.length} items from list-media endpoint`);
            } else if (result && Array.isArray(result)) {
                mediaList = result;
                log(`Loaded ${mediaList.length} items directly from list-media endpoint`);
            } else {
                throw new Error('Invalid data format from list-media');
            }
        } catch (firstError) {
            log('First endpoint failed:', firstError);
            ...
            try {
                ...
                log(`Loaded ${mediaList.length} media items from proxy`);
            } catch (secondError) {
                log('All endpoints failed. Using demo data.', secondError);
                
                // Use demo data if all endpoints fail
                mediaList = [
                    { id: 1, title: 'Demo Audio 1', description: 'Sample audio file', station: 'Demo', category: 'Audio' },
                    { id: 2, title: 'Demo Video', description: 'Sample video file', station: 'Demo', category: 'Video' },
                    { id: 3, title: 'Demo Image', description: 'Sample image file', station: 'Demo', category: 'Image' }
                ];
            }
        }
        ...
        const mediaBrowser = document.getElementById('mediaBrowser');
        if (mediaBrowser) {
            mediaBrowser.innerHTML += '<div class="empty-state">Failed to load from server, showing demo data</div>';
        }
    }
}
...
async function loadAssignments() {
    try {
        ...
        // Try loading from proxy endpoint
